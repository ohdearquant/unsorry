...
**Phase 2** is where that happens: point the swarm at a result that is *not* already formalised and let it drive a verified proof to it — directly, or by decomposition into sub-lemmas that re-enter the queue. That is when a solved theorem becomes a genuine new artifact rather than a test fixture. The machinery is built and live — affinity/gap selection, decomposition, and a statement-binding gate — and the swarm has proved its first one. (The plan: [docs/proposals/phase2-plan.md](docs/proposals/phase2-plan.md); open targets: [docs/targets.md](docs/targets.md).)

Two limits, stated plainly because this project runs on verification rather than optimism. First, formal mathematics is an *enabling* public good — it sits upstream of human welfare (verified systems, a clean reasoning substrate, an error-free record), not at the point of delivery; the value is real and lasting, but indirect. Second, the kernel certifies that a proof is *sound*, not that its statement *means* what was intended — closing that gap is the Phase-2 statement-binding gate ([ADR-011](docs/adrs/ADR-011-Statement-Binding-Gate.md), now live and red-team-proven). The number we judge ourselves by is not "theorems proved" but **"first theorem proved that was not already in mathlib"** — and as of [phase2-run-001](docs/metrics/phase2-run-001.md) that number is non-zero: the swarm proved Nicomachus's theorem `∑k³ = (∑k)²`, verified mathlib-absent, with the binding obligation holding.
