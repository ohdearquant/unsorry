# ADR-046: Decentralised CI Runner Architecture — Tiered Split with a Mandatory Cheap Central Re-check
...
**we decided for** a **tiered split**: push the expensive elaboration (`lake build --wfail` + axiom audit) onto the untrusted contributor (which `prove_local_verify()` already runs), and keep a **mandatory, cheap, trusted central re-check** as the *sole* load-bearing soundness gate, run at sampling probability p = 1 on every PR — a re-check that **never trusts a client-supplied `.olean`** but instead re-derives the statement from canonical goal source (ADR-018/ADR-011), re-elaborates the changed-module reverse-import closure from source against pinned, locally-built dependency oleans, then runs `leanchecker` + `axiom_audit` + statement-binding on that,
...
2. **The central re-check consumes no client artifact as a trusted input.** It (a) re-derives the statement from canonical goal source (ADR-018 create-only goals, ADR-011 binding), (b) re-elaborates the changed-module reverse-import closure from source against pinned dependency oleans, (c) runs `leanchecker`, (d) runs the serialised `axiom_audit` against the whitelist `{propext, Classical.choice, Quot.sound}`, (e) defeq-binds the proved term to the re-derived statement.
3. **The footgun is named and forbidden.** `leanchecker` replays the environment it is *given* — it trusts olean structure ("prone to crafted invalid `.olean` files") and does not re-derive the statement. So "leanchecker a client-supplied olean" is unsound on (i) crafted-invalid oleans and (ii) real proofs of weaker/renamed statements (the PR-#64 vacuity class). The design closes both by central source re-elaboration and central statement re-derivation; no code path may let a client olean reach `leanchecker` as a trusted input.
4. **Determinism is the precondition.** ADR-002 pinning + Lake lockfile + elan give the ADR-033 byte-identical-rebuild invariant, so the central re-elaboration verifies the same proof the contributor produced. Any global-impact change forces a full re-check rather than scoping down.
5. **The push-to-`main` backstop is unchanged.** Every merged olean is fully re-audited and re-replayed on push to `main` (no BASE_SHA), so even a defect in incremental scoping cannot silently admit unsound content.
...
| REF-2 | Decentralised CI runner architecture (implementation contract) | Specification | specs/SPEC-046-A-Decentralised-CI-Runner-Architecture.md |
