...
The central re-check **MUST NOT** consume any contributor-supplied compiled artifact (`.olean`, exported term, attestation, hash, or manifest field) as a *trusted* input. For each PR it **MUST**, on the trusted surface:
...
A contributor-supplied `.olean` reaching `leanchecker` as a trusted input is a **soundness defect**, not an optimisation. Rationale (ADR-046 soundness argument): `leanchecker` trusts olean structure and replays the statement it is given, so trusting client oleans is unsound on (i) crafted-invalid oleans and (ii) real proofs of weakened/renamed statements (the ADR-011 / PR-#64 vacuity class).
...
4. **Determinism:** under ADR-002 pinning, an unchanged module's olean rebuilds byte-identically (the ADR-033 invariant); divergence is a hard fail, never a silent accept.
5. **Cost measurement:** central runner-minutes per merged PR are measured baseline vs post-change; the reported saving is the elimination of the redundant unchanged-module build (honest framing per ADR-046), not a SETI-scale figure.

## 6. Phasing (contract milestones)

- **Phase 0 (this PR's scope is research only):** ADR-046 + this spec + the §5.1 regression tests authored red. No `gate-a.yml` control-flow change.
...
- **Dependency-olean cache trust assumption** — whether the central build restores ADR-045 verified-on-`main` dependency oleans or rebuilds them from source; if restored, the cache's provenance (a prior trusted Gate A run) must be stated as part of the TCB.
