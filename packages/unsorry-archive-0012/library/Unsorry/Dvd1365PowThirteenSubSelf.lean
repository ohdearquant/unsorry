...
theorem dvd_1365_pow_thirteen_sub_self (n : ℤ) : (1365 : ℤ) ∣ n ^ 13 - n := by
  ...
  have acc1 : ((3) * 5 : ℤ) ∣ n ^ 13 - n := co1.mul_dvd h0 h1
  have co2 : IsCoprime (((3) * 5) : ℤ) (7 : ℤ) := by
    rw [Int.isCoprime_iff_gcd_eq_one]; decide
  have acc2 : (((3) * 5) * 7 : ℤ) ∣ n ^ 13 - n := co2.mul_dvd acc1 h2
  have co3 : IsCoprime ((((3) * 5) * 7) : ℤ) (13 : ℤ) := by
    rw [Int.isCoprime_iff_gcd_eq_one]; decide
  have acc3 : ((((3) * 5) * 7) * 13 : ℤ) ∣ n ^ 13 - n := co3.mul_dvd acc2 h3
