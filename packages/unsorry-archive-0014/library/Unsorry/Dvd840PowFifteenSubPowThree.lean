...
theorem dvd_840_pow_fifteen_sub_pow_three (n : ℤ) : (840 : ℤ) ∣ n ^ 15 - n ^ 3 := by
  ...
  have acc1 : ((8) * 3 : ℤ) ∣ n ^ 15 - n ^ 3 := co1.mul_dvd h0 h1
  have co2 : IsCoprime (((8) * 3) : ℤ) (5 : ℤ) := by
    rw [Int.isCoprime_iff_gcd_eq_one]; decide
  have acc2 : (((8) * 3) * 5 : ℤ) ∣ n ^ 15 - n ^ 3 := co2.mul_dvd acc1 h2
  have co3 : IsCoprime ((((8) * 3) * 5) : ℤ) (7 : ℤ) := by
    rw [Int.isCoprime_iff_gcd_eq_one]; decide
  have acc3 : ((((8) * 3) * 5) * 7 : ℤ) ∣ n ^ 15 - n ^ 3 := co3.mul_dvd acc2 h3
  have keq : (((((8) * 3) * 5) * 7) : ℤ) = (840 : ℤ) := by norm_num
  rwa [keq] at acc3
